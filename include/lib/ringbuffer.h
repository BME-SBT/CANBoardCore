 ...
 * @tparam N the number of items inside the buffer
 */
template <typename T> class UnsafeAllocRingBuffer {

  public:
    explicit UnsafeAllocRingBuffer(int N): N(N), m_size(0), m_data(nullptr),m_head(m_data)  {
        m_data = new T[N];
        m_head = m_data;
    }

    virtual ~UnsafeAllocRingBuffer() {
        delete[] m_data;
    }
